...
public static class GameCheatList
{
	...
	public static void InitializeCheats(CheatsMenu cheatsMenu)
	{
		cheatsMenu.RegisterCheat(
			...
			new Cheat("start_dialog", "Start dialog", "Starts a dialog with the specified ID.", "cheat_start_dialog", (bool active, string input) =>
			{
				var inputArguments = cheatsMenu.ParseCheatArguments(input, 1);
				var args = inputArguments.args;

				if (!inputArguments.result)
				{
					cheatsMenu.Log("Invalid dialog ID. Usage: cheat_start_dialog <dialog_id>", CheatsMenu.LogLevel.Error);
					return;
				}

				string dialogId = args[0];
				PlayerVariables.Instance.Player.PlayerGui.DialogBox.StartDialog(dialogId);
				cheatsMenu.Log($"Started dialog with ID: {dialogId}", CheatsMenu.LogLevel.Result);
			},
			type: Cheat.CheatType.Input)
		);
	}
}
